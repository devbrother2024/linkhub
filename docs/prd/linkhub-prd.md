...
- [x] Toss Payments 자동결제(빌링) 기반으로 최초 결제 후 Billing Key를 저장하고, 성공 리다이렉트 단계에서 Server Action이 즉시 결제 승인 API를 호출해 Billing Key·결제 정보를 DB에 기록한다. (웹훅 미사용)
- [x] 결제 성공/실패 여부는 Toss Payments SDK 요청 직후 서버가 승인 API 응답을 확인해 `Subscription` 및 `User.planType`을 갱신하고, 결제 수단/주문번호/결제 금액/결제일/상태를 결제 이력 테이블에 적재한다.
- [x] 대시보드 상단 카드에 구독 상태, 구독 만료일, 구독 관리 페이지 이동 버튼을 노출해 사용자가 한눈에 상태를 확인하도록 한다.
- [x] 구독 관리 페이지에는 현재 플랜, 구독 시작일, 구독 만료일, 마지막 결제일, 재구독 버튼, 구독 취소 버튼, 결제 이력(결제 수단·결제 금액·주문번호·결제일·상태)을 제공한다.
- [x] 결제 이력 테이블은 정렬·필터 기능을 포함하고, 각 행에서 결제일·금액·상태를 중복 없이 노출해 고객 문의 대응에 활용 가능하도록 한다.
- [x] 구독 취소 시 사용자가 즉시 Free로 강등되지 않고 만료일까지 서비스 이용 가능하다는 안내와 타임라인을 UI에 명시한다.
- [x] 자동 갱신은 서울시간 매일 03시에 Vercel Cron으로 실행되며, Billing Key를 사용해 Toss Payments 예약 결제를 재시도한다.
- [x] 동일 스케줄러 로직을 로컬에서 재현할 수 있도록 `pnpm cron:test`(예시) 형태의 스크립트를 `package.json`에 등록해 개발 환경에서도 갱신 흐름을 검증한다.
- [x] 프로필/대시보드의 구독 카드와 결제 이력 섹션은 상태, 다음 결제일, 마지막 청구 결과를 카드 형태로 시각화해 사용자와 운영자가 동일 정보를 공유하게 한다.
